...
function initialize_payment(update_session = false) {
  ...
  payment_data.open(
    "POST",
    `${$("#request_host").val()}/Api/Initiate-Gateway-Session`,
    true
  );
...
const submitForm = () => {
  if (
    ...
    xhr.open(
      "POST",
      `${$("#request_host").val()}/Api/Get-Exibition-Packages-Full/Details/`
    );

    xhr.setRequestHeader("Authorization", $("#event_code").val());
    xhr.send(data);
  } else {
    $("#notification-draw").remove();
    $(".exhibition-form").prepend(`
